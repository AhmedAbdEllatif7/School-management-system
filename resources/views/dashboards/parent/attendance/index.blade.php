            ...
            <div class="col-md-12 mb-30">
                <div class="card card-statistics h-100">
                    <div class="card-body">
                        ...
                        <form method="GET" action = "{{route('reports.attendances')}}" autocomplete="off">
                            @csrf
                    ...
                    @isset($searchResult)
                        <div class="table-responsive">
                            <table id="datatable" class="table  table-hover table-sm table-bordered p-0" data-page-length="50"
                                ...
                                <tbody>
                                @foreach($searchResult as $son)
                                    <tr>
                                        <td>{{ $loop->index+1 }}</td>
                                        <td>{{$son->students->name}}</td>
                                        <td>{{$son->grade->name}}</td>
                                        <td>{{$son->section->name}}</td>
                                        <td>{{$son->attendance_date}}</td>
                                        <td>

                                            @if($son->attendance_status == 0)
